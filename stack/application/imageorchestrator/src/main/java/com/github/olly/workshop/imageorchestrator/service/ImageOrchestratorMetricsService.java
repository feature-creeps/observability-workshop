...
public class ImageOrchestratorMetricsService extends MetricsService {
    ...
    public void imageTransformed(Image sourceImage, Image transformedImage,
            TransformationRequest transformationRequest) {
        // we know very well that sourceId is a very bad example for a label as it has
        ...
        if (BUSINESS_METRICS_ENABLED) {
            Metrics.counter("application_images_transformed_total",
                    ...
                    "flip",
                    String.valueOf(transformationRequest.getTransformationTypes().contains(TransformationType.flip)),
                    "grayscale",
                    String.valueOf(
                            transformationRequest.getTransformationTypes().contains(TransformationType.grayscale)),
                    "resize",
                    String.valueOf(transformationRequest.getTransformationTypes().contains(TransformationType.resize)),
                    "rotate",
                    String.valueOf(transformationRequest.getTransformationTypes().contains(TransformationType.rotate)))
                    .increment();
        }
    }
}
