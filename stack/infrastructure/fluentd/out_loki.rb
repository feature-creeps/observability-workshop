  ...
  module Plugin
    ...
    class LokiOutput < Fluent::Plugin::Output # rubocop:disable Metrics/ClassLength
      ...
      def configure(conf)
        ...
        # BHO:
        #   at this point we write the configured labels to record_accessors
        #   this should be obsolete once we want to grab all existing labels
        @record_accessors = {}
      ...
      def write(chunk)
        ...
        print "POST body to loki: " + body.to_s + "\n"
      ...
      def line_to_loki(record)
        ...
        if record.is_a?(Hash)
          # BHO:
          # at this point we want to iterate over all existing labels and put them in the chunk_labels map
          #record.each do |key, value|
          #  new_key = key.gsub(%r{[.\-\/]}, '_')
          #  if value.is_a?(Hash)
          #    new_value = {}
          #    value.map(&new_value).flatten
          #  end
          #end
          record.each do |k, v|
            if v.is_a?String
              ...
              v = k if v.empty?
              new_key = k.gsub(%r{[.\-\/]}, '_')
              accessor = record_accessor_create(v)
              chunk_labels[new_key] = accessor.call(record)
              accessor.delete(record)
            else
              print "ignoring non string key/value: " + k.to_s + "/" + v.to_s + "\n"
            end
          end

          # BHO:
          # at this point we iterate over all configured labels and put them in the chunck_labels map
          # this should instead go over all existing labels
